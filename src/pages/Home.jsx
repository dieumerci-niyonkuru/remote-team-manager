...
const FEATURES = [
  { icon: <Zap size={24} />, title: 'Real-time Collaboration', desc: 'WebSocket-powered boards and live cursors keep every team member in sync, anywhere.' },
  { icon: <Target size={24} />, title: 'AI Task Intelligence', desc: 'AI prioritises, assigns, and surfaces the work that matters before it becomes a blocker.' },
  { icon: <MessageSquare size={24} />, title: 'Enterprise Chat', desc: 'Threaded channels, direct messages, reactions and rich media — all in one inbox.' },
  { icon: <Video size={24} />, title: 'Video Meetings', desc: 'One-click HD video calls with screen sharing, recording, and meeting notes.' },
  { icon: <Shield size={24} />, title: 'OKR Tracking', desc: 'Cascade objectives to every task and watch progress build in real time.' },
  { icon: <BarChart2 size={24} />, title: 'Advanced Analytics', desc: 'Velocity charts, burn-downs, and dashboards that turn data into clear decisions.' },
];
...
function FeatureCard({ feature }) {
  ...
  return (
    <div
      ...
      style={{
        background: 'var(--bg-card)', border: `1px solid ${hovered ? 'var(--brand)' : 'var(--border)'}`,
    ...
    >
      <div style={{
        width: 48, height: 48, borderRadius: 12, background: 'var(--brand-bg)',
      ...
      <h3 style={{ fontSize: 'clamp(15px,1.6vw,17px)', fontWeight: 800, color: 'var(--text)', margin: '0 0 8px' }}>{feature.title}</h3>
...
function CTASection() {
  ...
  return (
    <section style={{ padding: 'clamp(48px,8vw,80px) clamp(16px,4vw,24px)', background: 'var(--bg2)', borderTop: '1px solid var(--border)', textAlign: 'center' }}>
      <div style={{ maxWidth: 600, margin: '0 auto' }}>
        <h2 style={{ fontSize: 'clamp(24px,4vw,44px)', fontWeight: 800, color: 'var(--text)', letterSpacing: '-0.03em', lineHeight: 1.1, margin: '0 0 12px' }}>
          Start your free trial today
        </h2>
        <p style={{ fontSize: 'clamp(14px,1.6vw,15px)', color: 'var(--text3)', margin: '0 0 32px' }}>
        ...
        {sent ? (
          ...
          <form onSubmit={handleSubmit} style={{ display: 'flex', gap: 10, maxWidth: 440, margin: '0 auto', flexWrap: 'wrap', justifyContent: 'center' }}>
            ...
            <button type="submit" style={{
              padding: '12px 24px', borderRadius: 10, background: 'var(--brand)',
              color: '#fff', fontWeight: 700, fontSize: 14, border: 'none', cursor: 'pointer', whiteSpace: 'nowrap',
            }}>
...
export default function Home() {
  ...
  return (
    <div style={{ background: 'var(--bg)', color: 'var(--text)', overflowX: 'hidden', minHeight: '100vh', fontFamily: 'inherit' }}>
      ...
      <section style={{ position: 'relative', minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 'clamp(80px,12vh,160px) clamp(16px,4vw,24px) clamp(40px,6vw,80px)', overflow: 'hidden', textAlign: 'center' }}>
        <div style={{ position: 'relative', zIndex: 1, maxWidth: 800 }}>
          <div style={{
            ...
            borderRadius: 24, padding: '5px 14px', marginBottom: 'clamp(20px,3vw,28px)',
          ...
          <h1 style={{
            fontSize: 'clamp(32px,7vw,80px)', fontWeight: 800, letterSpacing: '-0.04em',
          ...
          <p style={{
            fontSize: 'clamp(14px,2vw,18px)', lineHeight: 1.65, color: 'var(--text2)',
            maxWidth: 560, margin: '0 auto clamp(24px,4vw,36px)',
          ...
          <div style={{ display: 'flex', gap: 12, justifyContent: 'center', flexWrap: 'wrap', marginBottom: 'clamp(36px,5vw,56px)', animation: 'fadeSlideUp 0.9s ease 0.4s both' }}>
            <button
              onClick={() => navigate('/register')}
              style={{
                padding: 'clamp(12px,2vw,14px) clamp(20px,3vw,28px)', borderRadius: 10,
                background: 'var(--brand)', color: '#fff', fontWeight: 700,
                fontSize: 'clamp(14px,1.5vw,15px)', border: 'none', cursor: 'pointer',
                boxShadow: 'var(--shadow-brand)', transition: 'transform 0.15s',
              }}
            ...
            <button
              ...
              style={{
                padding: 'clamp(12px,2vw,14px) clamp(16px,2.5vw,24px)', borderRadius: 10,
                background: 'var(--bg2)', border: '1px solid var(--border)',
                color: 'var(--text)', fontWeight: 700, fontSize: 'clamp(14px,1.5vw,15px)',
      ...
      <section style={{ padding: 'clamp(48px,8vw,100px) clamp(16px,4vw,24px)', background: 'var(--bg)' }}>
        <div style={{ maxWidth: 1000, margin: '0 auto' }}>
          <div style={{ textAlign: 'center', marginBottom: 'clamp(32px,5vw,52px)' }}>
            ...
            <h2 style={{ fontSize: 'clamp(22px,4vw,44px)', fontWeight: 800, letterSpacing: '-0.03em', margin: '0 0 12px', color: 'var(--text)' }}>
              One platform, every workflow.
            </h2>
            <p style={{ fontSize: 'clamp(14px,1.6vw,15px)', color: 'var(--text3)', maxWidth: 480, margin: '0 auto' }}>
      ...
      {/* DEMO */}
      <section style={{ padding: 'clamp(48px,8vw,100px) clamp(16px,4vw,24px)', background: 'var(--bg2)', textAlign: 'center' }}>
        <div style={{ maxWidth: 1000, margin: '0 auto' }}>
          ...
          <h2 style={{ fontSize: 'clamp(22px,4vw,42px)', fontWeight: 800, letterSpacing: '-0.03em', margin: '0 0 8px', color: 'var(--text)' }}>
            Your whole operation, one screen.
          </h2>
          <p style={{ fontSize: 'clamp(14px,1.6vw,15px)', color: 'var(--text3)', margin: '0 clamp(24px,4vw,40px)' }}>
      ...
      <section style={{ padding: 'clamp(48px,8vw,100px) clamp(16px,4vw,24px)', background: 'var(--bg)' }}>
        <div style={{ maxWidth: 1000, margin: '0 auto' }}>
          <div style={{ textAlign: 'center', marginBottom: 'clamp(28px,4vw,44px)' }}>
            ...
            <p style={{ fontSize: 'clamp(14px,1.6vw,15px)', color: 'var(--text3)' }}>Don't just take our word for it.</p>
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(min(260px, 100%), 1fr))', gap: 'clamp(12px,2vw,16px)' }}>
      ...
      {/* PRICING */}
      <section style={{ padding: 'clamp(48px,8vw,100px) clamp(16px,4vw,24px)', background: 'var(--bg2)' }}>
        <div style={{ maxWidth: 1000, margin: '0 auto' }}>
          <div style={{ textAlign: 'center', marginBottom: 'clamp(32px,5vw,52px)' }}>
            ...
            <p style={{ fontSize: 'clamp(14px,1.6vw,15px)', color: 'var(--text3)' }}>
      ...
      {videoModalOpen && (
        <div
          ...
          <div onClick={e => e.stopPropagation()} style={{ background: 'var(--bg2)', border: '1px solid var(--border)', borderRadius: 16, padding: 'clamp(28px,4vw,40px)', maxWidth: 440, width: '100%', textAlign: 'center' }}>
