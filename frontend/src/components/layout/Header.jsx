          ...
          <div className="header-actions" style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
            ...
            {isAuth ? (
              <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                <div style={{ width: '32px', height: '32px', borderRadius: '50%', background: 'linear-gradient(135deg, #4f46e5, #8b5cf6)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'white', fontWeight: 'bold', cursor: 'pointer' }} onClick={() => navigate('/dashboard')}>
                  {user?.first_name?.[0]}{user?.last_name?.[0]}
                </div>
                <button className="btn-secondary" onClick={handleLogout} style={{ background: 'transparent', border: '1px solid #4f46e5', padding: '6px 16px', borderRadius: '8px', color: 'white' }}>{t.logout}</button>
              </div>
            ) : (
              <div style={{ display: 'flex', gap: '10px' }}>
                <Link to="/login" className="btn-secondary" style={{ background: 'transparent', border: '1px solid #4f46e5', padding: '6px 16px', borderRadius: '8px', color: 'white' }}>{t.login}</Link>
                <Link to="/register" className="btn-primary" style={{ background: '#4f46e5', padding: '6px 16px', borderRadius: '8px', color: 'white' }}>{t.register}</Link>
              </div>
            )}
          </div>

          <button className="mobile-menu-btn" onClick={() => setMobileOpen(!mobileOpen)} style={{ display: 'none', background: 'none', border: 'none', fontSize: '28px', color: 'white', cursor: 'pointer' }}>☰</button>
        </div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
              ...
              {!isAuth && (
                <div style={{ display: 'flex', gap: '10px', marginTop: '10px' }}>
                  <Link to="/login" onClick={() => setMobileOpen(false)} style={{ flex: 1, textAlign: 'center', background: 'transparent', border: '1px solid #4f46e5', padding: '12px', borderRadius: '8px', color: 'white' }}>{t.login}</Link>
                  <Link to="/register" onClick={() => setMobileOpen(false)} style={{ flex: 1, textAlign: 'center', background: '#4f46e5', padding: '12px', borderRadius: '8px', color: 'white' }}>{t.register}</Link>
                </div>
              )}
              {isAuth && (
